...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagFiltrate)
    {
        info->event[info->num] = EVENT_Filtrate;
        info->num++;
        dataPoints->valueFiltrate = gizStandardDecompressionValue(Filtrate_BYTEOFFSET,Filtrate_BITOFFSET,Filtrate_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
        
    if(0x01 == issuedData->attrFlags.flagTemplate_write)
    {
        info->event[info->num] = EVENT_Template_write;
        info->num++;
        dataPoints->valueTemplate_write = gizX2YFloat(Template_write_RATIO,  Template_write_ADDITION, (float)(exchangeBytes(issuedData->attrVals.valueTemplate_write)));
    }
        
    if(0x01 == issuedData->attrFlags.flagFeed_time)
    {
        info->event[info->num] = EVENT_Feed_time;
        info->num++;
        dataPoints->valueFeed_time = gizX2Y(Feed_time_RATIO,  Feed_time_ADDITION, exchangeBytes(issuedData->attrVals.valueFeed_time));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueFiltrate != cur->valueFiltrate)
    {
        GIZWITS_LOG("valueFiltrate Changed\n");
        ret = 1;
    }
    if(last->valueTemplate_write != cur->valueTemplate_write)
    {
        GIZWITS_LOG("valueTemplate_write Changed\n");
        ret = 1;
    }
    if(last->valueFeed_time != cur->valueFeed_time)
    {
        GIZWITS_LOG("valueFeed_time Changed\n");
        ret = 1;
    }
    if(last->valueFeed_food != cur->valueFeed_food)
    {
        GIZWITS_LOG("valueFeed_food Changed\n");
        ret = 1;
    }

    if(last->valueTemplate_read != cur->valueTemplate_read)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueTemplate_read Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(Filtrate_BYTEOFFSET,Filtrate_BITOFFSET,Filtrate_LEN,(uint8_t *)devStatusPtr,dataPoints->valueFiltrate);
    gizStandardCompressValue(Feed_food_BYTEOFFSET,Feed_food_BITOFFSET,Feed_food_LEN,(uint8_t *)devStatusPtr,dataPoints->valueFeed_food);
    ...
    devStatusPtr->valueTemplate_write = exchangeBytes(gizY2XFloat(Template_write_RATIO,  Template_write_ADDITION, dataPoints->valueTemplate_write));     
    devStatusPtr->valueFeed_time = exchangeBytes(gizY2X(Feed_time_RATIO,  Feed_time_ADDITION, dataPoints->valueFeed_time)); 
    devStatusPtr->valueTemplate_read = exchangeBytes(gizY2XFloat(Template_read_RATIO,  Template_read_ADDITION, dataPoints->valueTemplate_read));     
